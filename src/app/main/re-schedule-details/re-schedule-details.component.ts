...
@Component({
	selector: 'app-re-schedule-details',
	templateUrl: './re-schedule-details.component.html',
	styleUrls: ['./re-schedule-details.component.scss'],
	providers: [
		{
...
export class ReScheduleDetailsComponent implements OnInit {
	...
	dataChange() {
		if (this.endDate && this.startDate && this.busNumber) {
			this.getTripData();
			this.overAllData = {};
			let tempObject = {};
			this.selectedDates.forEach(date => {
				let tempObjectOne = {};
				let bus = this.busNumber;
	...
	getTripData(){
		...
		this.busData.forEach(element => {
			if (element.auto_inc_id == this.busNumber) {
				let noTrips = element.no_trips;
				let tempData = [];
				for (let index = 0; index < noTrips; index++) {
					tempData.push({ value: index+1})
				}
				this.tripData = tempData;
			}
		});
	...
	onSubmit(form) {
		...
		for (let index = 0; index < Keys.length; index++) {
			...
			if (moment(dateKey, "D-M-YYYY").diff(moment(new Date(), "D-M-YYYY"), "days") >= 0) {
				...
				const busKey = this.busNumber
				let tempData = this.overAllData[dateKey][busKey];
				requestPreData.push({
					bus_details: busKey,
					date: dateKey,
					trip_no: JSON.stringify(this.overAllData[dateKey][busKey].noTrips) ,
